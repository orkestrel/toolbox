...
| `TerminalBridge`   | class | Bridge a terminal manager onto the wire, owning the shared route options, the token gate, and the bound GET stream and POST answer handlers that `createTerminalRoutes` projects. |
...
The POST endpoint takes `{ id, values }`, admits the body only when `id` is a nonempty string and `values` passes `@orkestrel/form`'s `isFormValues`, and returns the manager's own answer `Result` as a JSON body — `200` accepted, `422` an `'unknown'` / `'rejected'` outcome, `404` a `'terminal'` outcome. The body is what carries a rejection's per-field `errors` back to the answering client, so `PromptClient` can re-render the failed fields instead of guessing from a status code.
