...
| `isColumnPrimitive`    | const    | `ColumnPrimitive`    | Narrows an unknown value to a `ColumnPrimitive`.                                                                                                                                                                                                                                                                                                                                                |
...
| `EndpointHandler`          | type      | `(args: Readonly<Record<string, unknown>>, context: ToolContext) => Promise<unknown> \| unknown`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | Represents the handler `EndpointDefinition.execute` implements — it mirrors `@orkestrel/tool`'s `ToolOptions.execute` signature exactly, so `createEndpointTool` forwards the execution context it receives to the handler unchanged and a handler that reads `context.signal` observes the caller's cancellation. |
...
18. **A typed upstream failure re-surfaces in-process as a typed `ToolboxError`, never passes through raw.** `createDatabaseTool` catches a `@orkestrel/database` `DatabaseError` and re-throws a typed `DATABASE` `ToolboxError` carrying the original `DatabaseErrorCode` in `context.code` (`inferDatabaseCode`); `createRelationTool` does the same for a `@orkestrel/relation` `RelationError` → `RELATION` (`inferRelationCode`, checked first) and, underneath it, a `DatabaseError` → `DATABASE` (mirroring the database tool's own mapping) — so an in-process catch or direct `tool.execute(args, context)` call sees exactly one of `TOOL` (this tool's own guards: malformed args, unknown manager/model/database/driver), `RELATION`, or `DATABASE`, never an unwrapped upstream error. A `ToolboxError` already thrown by this tool's own guards passes through unwrapped (never re-mapped a second time). A `ToolManagerInterface.execute` registry caller does not receive that code or context; it receives only the flattened message string.
...
const summary = await tool.execute({}, { signal: new AbortController().signal }) // native runner checkpoints through `store`
...
The handler takes the caller's execution context as its second parameter, the same one a
`@orkestrel/tool` handler takes. `createEndpointTool` forwards that context unchanged whichever way
it handled the args — parsed against the inferred schema, or passed through raw under
`{ validate: false }` — so a handler that reads `context.signal` observes the caller's cancellation.
The preceding fence declares `args` alone, which still satisfies `EndpointHandler`: a handler reads
the signal only where it has cancellable work to abandon.
