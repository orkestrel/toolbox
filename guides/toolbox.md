...
| API                    | Kind  | Summary                                                                                                                                                                                                                                                                                                                                                                                                                              |
| ---------------------- | ----- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `agentToolShape`       | const | Describes the shape of `AgentToolArguments` — `createAgentTool`'s advertised `parameters`.                                                                                                                                                                                                                                                                                                                                           |
| `taskDraftShape`       | const | Describes the shape of a `TaskDraft` — identical to a strict task shape except that `id` and `name` are optional.                                                                                                                                                                                                                                                                                                                    |
| `phaseDraftShape`      | const | Describes the shape of a phase in a draft workflow — identical to a strict phase shape except that `id` and `name` are optional and each task takes `taskDraftShape`.                                                                                                                                                                                                                                                                |
| `workflowDraftShape`   | const | Describes the shape of a draft workflow — identical to a strict workflow shape except that `id` and `name` are optional at the workflow, phase, and task levels, so a small model can omit every identity string and let the tool synthesize them positionally.                                                                                                                                                                      |
| `stepShape`            | const | Describes the shape of one flat step — `{ name }` — the building block of `workflowStepsShape`.                                                                                                                                                                                                                                                                                                                                      |
| `workflowStepsShape`   | const | Describes the flat authoring shape `createWorkflowTool` advertises as its `parameters` — the simplest surface a small model can fill: `{ name?, steps: [{ name }] }`.                                                                                                                                                                                                                                                                |
...
| `describeToolShape`    | const | Describes the shape of `DescribeToolArguments` — `createDescribeTool`'s advertised `parameters`.                                                                                                                                                                                                                                                                                                                                     |
| `promptToolShape`      | const | Describes the shape of `createPromptTool`'s call arguments — `to` names the terminal identity and `schema` carries the complete multi-field form document.                                                                                                                                                                                                                                                                           |
| `answerToolShape`      | const | Describes the shape of `createAnswerTool`'s call arguments — discriminated by `operation`: `'pending'` lists the forms addressed to this tool's terminal, while `'answer'` resolves one by `id` with a complete `values` record.                                                                                                                                                                                                     |
| `databaseToolShape`    | const | Describes the shape of `createDatabaseTool`'s call arguments — discriminated by `operation` into the database operations `'create'`, `'tables'`, `'get'`, `'records'`, `'count'`, `'aggregate'`, `'add'`, `'set'`, `'update'`, `'remove'`, and `'destroy'`.                                                                                                                                                                          |
| `columnPrimitiveShape` | const | Describes a `ColumnPrimitive` literal — the leaf `columnSpecShape` wraps.                                                                                                                                                                                                                                                                                                                                                            |
| `columnSpecShape`      | const | Describes a `ColumnSpec` — a bare `columnPrimitiveShape`, or `{ primitive, optional }`.                                                                                                                                                                                                                                                                                                                                              |
| `tableSpecShape`       | const | Describes a `TableSpec` — table name to `{ columns }`, each column a `columnSpecShape`.                                                                                                                                                                                                                                                                                                                                              |
| `keyShape`             | const | Describes one key value for the database tool and the relation tool — a string or number; the array form (multiple keys, positional) resolves first, so an array argument is read as many keys rather than one.                                                                                                                                                                                                                      |
| `rowShape`             | const | Describes a loose row — a flat object of column name to JSON value.                                                                                                                                                                                                                                                                                                                                                                  |
| `rowsShape`            | const | Describes one or many loose rows — the array form resolves first, so an array argument is read as many rows rather than one.                                                                                                                                                                                                                                                                                                         |
| `conditionShape`       | const | Describes one serialized where condition — `values` is always an array, even for a single-value operator.                                                                                                                                                                                                                                                                                                                            |
| `orderShape`           | const | Describes one sort term — a `column` to sort by and the `direction` to sort it in.                                                                                                                                                                                                                                                                                                                                                   |
| `queryShape`           | const | Describes the serialized query form — conditions, order, and pagination.                                                                                                                                                                                                                                                                                                                                                             |
| `relationToolShape`    | const | Describes the shape of `createRelationTool`'s call arguments — discriminated by `operation` into the relation operations `'load'`, `'find'`, `'link'`, `'unlink'`, and `'links'`.                                                                                                                                                                                                                                                    |
| `singleKeyShape`       | const | Describes a single row key (not an array) — used by `'link'` / `'unlink'` / `'links'`, which address exactly one owning row.                                                                                                                                                                                                                                                                                                         |
| `includeShape`         | const | Describes the flat dot-path relation include list, expanded through `expandInclude`.                                                                                                                                                                                                                                                                                                                                                 |
| `managerShape`         | const | Describes which registered relation manager to address — omitted resolves to the sole registered manager.                                                                                                                                                                                                                                                                                                                            |
| `inferToolShape`       | const | Describes the shape of `createInferTool`'s call arguments — one or more example `samples` to infer a JSON Schema from, plus per-call `format` / `enum` toggles and an optional `candidates` array to check against the inferred schema.                                                                                                                                                                                              |
...
A `Shape` cell holds an interface's data members as bare names in braces, `?` marking an optional member and `plus` introducing its call-signature members, and a type alias's own type literal with a union's arms escaped as `\|`. In a constants row a `Shape` cell holds the constant's declared type.

| API                     | Kind      | Shape                                                                    | Summary                                                                                                                                                                                                                                                                                                                                                                                  |
| ----------------------- | --------- | ------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `createTerminalRoutes`  | function  |                                                                          | Builds the GET SSE stream and POST answer routes that bridge a terminal manager onto the wire.                                                                                                                                                                                                                                                                                           |
