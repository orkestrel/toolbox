...
| `createAgentTool`               | function | Build the sub-agent delegation `ToolInterface` — resolves + runs one seeded agent through an `AgentRegistryInterface`, depth/cycle guarded, with an optional pluggable `ConversationStoreInterface`.                                                                                                                                                                                                                           |
...
| `includeShape`         | const | Flat dot-path relation include list, expanded through `expandInclude`.                                                                                                                    |
...
| `WorkflowSteps`            | interface | `{ name?, steps }` — the FLAT authoring blob `createWorkflowTool` advertises; `name` is also the deterministic workflow id, and each step → a one-task phase through `expandSteps`.                                                                                                                                         |
