...
`createWorkflowTool` and `createWorkspaceTool` own their FULL handler logic (the workflow half ported byte-faithfully from `@orkestrel/workflow` ahead of the upstream cleanup that drops the authoring surface from that package; the workspace half ported from `@orkestrel/agent` before the workspace domain was extracted to `@orkestrel/workspace`, which now owns the files and editing surface this tool drives — this package is the defining home for both) and layer a pluggable `store` slot on top; `createAgentTool` (sub-agent delegation over an `AgentRegistryInterface`) layers its OWN pluggable `store` slot too — a `ConversationStoreInterface` (`@orkestrel/agent`) that persists each delegation's sub-agent conversation on settle. All three tools additionally advertise a lean `summary` (`@orkestrel/tool`'s `ToolInterface.summary` / `ToolManagerInterface.definitions()` projection) in place of their full teaching `description`; `createDescribeTool` is the net-new on-demand expansion seam — given a registered tool's name, it returns that tool's full `description`. The two workflow-function adapters `createToolFunction` / `createAgentFunction` compose a registered runtime tool or a live agent into a workflow's `functions` registry; the authoring umbrella (`WorkflowSteps` / `WorkflowDraft` shapes, `createWorkflowDraftContract`, `expandSteps` / `completeDraft`, `workflowToolSummary`, `MAX_WORKFLOW_DEPTH`) is what lets a small model author a whole tree in one call.
...
| API                             | Kind     | Summary                                                                                                                                                                                                                                                                                                                                                           |
| ------------------------------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `createToolFunction`            | function | Wrap a registered runtime tool as a `WorkflowFunction` — the OPT-IN adapter for a `function`-form task that runs an `@orkestrel/tool` tool BY NAME.                                                                                                                                                                                                               |
...
| API              | Kind     | Summary                                                                                                                                                                                                                                                                                                                    |
| ---------------- | -------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ToolboxError`   | class    | Carries a `ToolboxErrorCode` (`TOOL` / `DEPTH` / `DEADLOCK` / `EXPIRE` / `ANSWER` / `DATABASE` / `RELATION`) + an optional `context` — this package's general tool-call error, thrown by `createAgentTool` / `createDescribeTool` / `createPromptTool` / `createAnswerTool` / `createDatabaseTool` / `createRelationTool`. |
...
Every `create*Tool` factory returns a plain `ToolInterface` (`@orkestrel/tool`'s type — its method surface is documented in [`tool.md`](tool.md), not re-documented here), and `createAgentFunction` / `createToolFunction` return a plain `WorkflowFunction` (`@orkestrel/workflow`'s own type — see [`workflow.md`](workflow.md)). The `WorkspaceManagerInterface` / `WorkflowRunnerInterface` / `AgentRegistryInterface` a caller supplies are likewise defined and documented upstream. `DefinitionStoreInterface` is the point-access persistence seam `MemoryDefinitionStore` / `DatabaseDefinitionStore` implement; the lifecycle classes expose their minimal orchestration methods directly.
...
11. **`ToolboxError` is this package's seven-code general tool-call error.** It carries a machine-readable `ToolboxErrorCode` + an optional `context` bag and is always thrown, never returned as `{ error }`, under AGENTS' “narrow untrusted input with guards” rule: `TOOL` covers malformed calls and package-owned resolution/configuration guards (including a malformed workspace operation; unknown tools, terminals, databases, drivers, relation managers, or models; invalid prompt choices/include paths; readonly mutations; invalid infer/endpoint inputs); `DEPTH` rejects an over-deep or cyclic `createAgentTool` delegation; `DEADLOCK` and `EXPIRE` re-surface `createPromptTool` asks that would form a prompt cycle or expire unanswered; `ANSWER` covers `createAnswerTool` calls whose prompt is unknown/already settled or whose answer is rejected; `DATABASE` re-surfaces a typed `DatabaseError` from `createDatabaseTool` or `createRelationTool` with its granular code in `context.code`; and `RELATION` re-surfaces a typed `RelationError` from `createRelationTool` with its granular code in `context.code`. `@orkestrel/workflow`'s `WorkflowError` and `@orkestrel/workspace`'s `WorkspaceError` still cover their genuine domain failure paths and pass through unchanged (`WorkspaceError`: `MODALITY` / `PATTERN` / `RANGE`; it has no `TOOL` arm). The typed error's `code` and `context` are readable only through an in-process catch, including around a direct `tool.execute(args)` call; a `ToolManagerInterface.execute` registry caller receives only the flattened message string.

12. **The workflow-function adapters are OPT-IN, composed into the caller's own registry.** `createToolFunction(tools, name)` and `createAgentFunction(agent, options?)` each return a plain `WorkflowFunction` (`@orkestrel/workflow`) — the pure workflow runner carries no knowledge of tools or agents itself; a caller composes either adapter into its own `WorkflowOptions.functions` registry like any other behavior (`{ publish: createToolFunction(tools, 'publish') }`). `createToolFunction` resolves the live tool with `tools.tool(name)`, calls `tool.execute(controller.input)` directly, and on failure throws an `Error` whose `cause` is the genuine thrown value — preserving a typed error in-process without a lossy registry-envelope round trip. `createAgentFunction` folds the task's cancellation into the wrapped agent's run (an already-aborted `controller.signal` cancels up front; otherwise a one-shot listener fires `agent.abort` on the task's own cancellation) and, when `options.runner` is supplied, BINDS a depth/cycle-aware `createWorkflowTool` onto the agent's `context.tools` under `WORKFLOW_TOOL_NAME` — the propagation seam letting the wrapped agent author + run a NESTED workflow.
...
19. **`DatabaseToolOptions.readonly` gates every mutating operation up front.** When `true`, `createDatabaseTool` throws a typed `TOOL` `ToolboxError` for `'create'` / `'add'` / `'set'` / `'update'` / `'remove'` / `'migrate'` / `'destroy'` BEFORE resolving a database or touching storage — the non-mutating operations (`'tables'` / `'get'` / `'records'` / `'count'` / `'aggregate'`) are unaffected. There is no equivalent gate on `createRelationTool` — its `'link'` / `'unlink'` writes are ungated (relation-tool callers rely on the underlying database's own access controls, if any).
...
```ts
import { createAgentFunction, createToolFunction } from '@orkestrel/toolbox'
import { createToolManager } from '@orkestrel/tool'
import { createWorkflowRunner } from '@orkestrel/workflow'
import type { WorkflowDefinition } from '@orkestrel/workflow'
