...
2. **One tool = one behavior; the runtime supplies the envelope, this package supplies the handler.** Each `create*Tool` factory returns a plain `ToolInterface` (`@orkestrel/tool`) — `name` / `description` / `parameters` / `execute`. The handler PARSES the model-supplied `args` against a compiled [contract](contract.md), dispatches, and either RETURNS a plain value on success or THROWS a typed error on every failure path (AGENTS §14's universal tool-handler contract) — it never builds a `ToolResult` itself. The `ToolManagerInterface.execute` (`@orkestrel/tool`) performs the ONE canonical wrap — `{ id, name, value }` on a return, `{ id, name, error }` on a throw, ISOLATED so nothing escapes the run — so a failure appears EXACTLY ONCE, identically, over both the agent loop and MCP.
...
11. **`ToolboxError` is this package's seven-code general tool-call error.** It carries a machine-readable `ToolboxErrorCode` + an optional `context` bag and is always thrown, never returned as `{ error }` (AGENTS §14): `TOOL` covers malformed calls and package-owned resolution/configuration guards (unknown tools, terminals, databases, drivers, relation managers, or models; invalid prompt choices/include paths; readonly mutations; invalid infer/endpoint inputs); `DEPTH` rejects an over-deep or cyclic `createAgentTool` delegation; `DEADLOCK` and `EXPIRE` re-surface `createPromptTool` asks that would form a prompt cycle or expire unanswered; `ANSWER` covers `createAnswerTool` calls whose prompt is unknown/already settled or whose answer is rejected; `DATABASE` re-surfaces a typed `DatabaseError` from `createDatabaseTool` or `createRelationTool` with its granular code in `context.code`; and `RELATION` re-surfaces a typed `RelationError` from `createRelationTool` with its granular code in `context.code`. `@orkestrel/workflow`'s `WorkflowError` and `@orkestrel/agent`'s `WorkspaceError` still cover the workflow tool's and workspace tool's own failure paths and pass through unchanged.
...
18. **A typed upstream failure re-surfaces as a typed `ToolboxError`, never passes through raw.** `createDatabaseTool` catches a `@orkestrel/database` `DatabaseError` and re-throws a typed `DATABASE` `ToolboxError` carrying the original `DatabaseErrorCode` in `context.code` (`databaseToolCode`); `createRelationTool` does the same for a `@orkestrel/relation` `RelationError` → `RELATION` (`relationToolCode`, checked FIRST) and, underneath it, a `DatabaseError` → `DATABASE` (mirroring the database tool's own mapping) — so a relation-tool caller sees exactly one of `TOOL` (this tool's own guards: malformed args, unknown manager/model/database/driver), `RELATION`, or `DATABASE`, never an unwrapped upstream error. An `ToolboxError` already thrown by this tool's own guards passes through UNWRAPPED (never re-mapped a second time).
...
- [`agent.md`](agent.md) — the `WorkspaceManagerInterface` / `WorkspaceStoreInterface` the workspace tool drives, and the `AgentRegistryInterface` / `AgentInterface` the agent tool and `createAgentFunction` resolve and run.
