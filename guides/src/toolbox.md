...
- [`tests/src/core/helpers.test.ts`](../../tests/src/core/helpers.test.ts) — the reusable workflow, terminal, database, and relation helpers; database coverage includes `TableSpec` expansion, strict `DatabaseDefinition` validation for `primary` / `indexes` / finite `version`, obsolete-field rejection, `queryOf` connector defaults, and `clampQuery` probe limits.
