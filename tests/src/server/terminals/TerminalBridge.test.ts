...
// tests/src/server/terminals/TerminalBridge.test.ts — mirrors src/server/terminals/TerminalBridge.ts.
// The bridge's own handlers are invoked DIRECTLY, through the routes `createTerminalRoutes`
// projects, against a real `createTerminalManager` (`@orkestrel/terminal`) for deterministic
// coverage (AGENTS' no-mocks rule); an injected, controllable `TimerHandler` drives the SSE
// keepalive without wall-clock timing.
...
describe('TerminalBridge — the token gate and the bound GET / POST handlers', () => {
	...
	it('GET streams a replayed pending prompt, a live pending event, and an expire event; keepalive fires; abort ends the stream', async () => {
		...
		const manager = createTerminalManager()
		manager.add('assistant', { timeout: 5, timer: timer.timer })
		const routes = createTerminalRoutes(manager, { timer: timer.timer, keepalive: 1000 })
		const get = findRoute(routes, 'GET')
...
describe('self-heal: consumer-side stream close (no signal abort) tears down on next event / keepalive tick', () => {
	...
	it('double-teardown through both paths — self-heal first, then a later signal abort — stays clean and does not throw', async () => {
		...
		// The request signal aborts AFTER self-heal already tore everything down. Because teardown
		// removed its own `abort` listener during the self-heal pass, this abort is a no-op — no
		// throw, no double-detach errors, no re-invocation of teardown's body.
