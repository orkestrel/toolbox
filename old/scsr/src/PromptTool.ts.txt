 ...
 *
 * @remarks
 * Dispatches prompt operations to a PromptFormInterface, and template
 * operations to a TemplateManagerInterface. When `stores` is provided,
 * templates are persisted to disk and loaded on `init()`.
 */
export class PromptTool implements PromptToolInterface {
		...
		if (id === undefined) {
			if (this.#stores) {
				for (const store of this.#stores.stores()) {
					if (store.writable) {
						for (const tpl of this.#templates.templates()) {
							store.remove(tpl.id)
						}
					}
				}
	...
	async execute(args: Readonly<Record<string, unknown>>): Promise<PromptToolResult> {
	...
	async #handleRegister(
		...
		await this.#persist(templateArg)
		return this.#success('register', { id: templateArg.id, registered: true }, start)
	}

	#handleFill(args: Readonly<Record<string, unknown>>, start: number): PromptToolResult {
		const id = args['id']
		if (typeof id !== 'string') {
	...
	#handleValidate(args: Readonly<Record<string, unknown>>, start: number): PromptToolResult {
		const id = args['id']
		if (typeof id !== 'string') {
	...
	#handleRemove(args: Readonly<Record<string, unknown>>, start: number): PromptToolResult {
		const id = args['id']
		if (typeof id !== 'string') {
	...
	#resolveValidationRules(value: unknown): ValidationRules | undefined {
		...
		const required = parseBooleanField(value, 'required')
		const minimum = parseNumberField(value, 'minimum')
		const maximum = parseNumberField(value, 'maximum')
		const pattern = parseStringField(value, 'pattern')
		const email = parseBooleanField(value, 'email')
		const url = parseBooleanField(value, 'url')
		const numeric = parseBooleanField(value, 'numeric')
		const integer = parseBooleanField(value, 'integer')
		const alphanumeric = parseBooleanField(value, 'alphanumeric')
		if (required !== undefined) rules.required = required
		if (minimum !== undefined) rules.minimum = minimum
		if (maximum !== undefined) rules.maximum = maximum
		if (pattern !== undefined) rules.pattern = pattern
		if (email !== undefined) rules.email = email
		if (url !== undefined) rules.url = url
		if (numeric !== undefined) rules.numeric = numeric
		if (integer !== undefined) rules.integer = integer
		if (alphanumeric !== undefined) rules.alphanumeric = alphanumeric
		if (Object.keys(rules).length === 0) return undefined
		return rules as ValidationRules
	}
	...
	#success(operation: PromptOperation, output: unknown, start: number): PromptToolResult {
		return { operation, ok: true, output, error: undefined, duration: performance.now() - start }
	}
	...
	async #persist(template: PromptTemplate): Promise<void> {
