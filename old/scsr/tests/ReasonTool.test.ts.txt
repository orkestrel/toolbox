...
import type { MCPStoreManagerInterface } from '@scsr/core'
import { createReason, createQuantitativeReasoner, ReasonTool } from '@scsr/core'
import { createReasonDefinitionImporter } from '@scsr/server'
import {
	emptySubject,
	quantitativeDef,
	sumGroup,
...
import { extractProperty } from '@scsr/core'
...
describe('ReasonTool', () => {
	...
	it('parameters definition has type enum', async () => {
		...
		const props = extractProperty(defSchema, 'properties')
		const typeField = extractProperty(props, 'type')
		expect(extractProperty(typeField, 'enum')).toEqual([
			'quantitative',
			'logical',
			'symbolic',
			'inferential',
	...
	it('throws descriptive error for invalid definition type', async () => {
		...
		).rejects.toThrow('No reasoner registered for type "nonsense"')
	})
	...
	describe('parameters schema', () => {
		...
		it('includes subjects field in parameters', async () => {
			...
			expect(extractProperty(subjects, 'type')).toBe('array')
			expect(extractProperty(subjects, 'items')).toEqual({
				type: 'object',
				additionalProperties: true,
			})
			expect(extractProperty(extractProperty(subjects, 'items'), 'properties')).toBeUndefined()
			expect(extractProperty(extractProperty(subjects, 'items'), 'required')).toBeUndefined()
		})
	})
...
describe('ReasonTool import', () => {
	...
	function createToolWithImport(
		extra?: Partial<{
			name: string
			summary: string
			description: string
			stores: MCPStoreManagerInterface
		}>,
	) {
		const tool = new ReasonTool({
			name: extra?.name ?? 'calc',
			summary: extra?.summary ?? 'Calc',
			description: extra?.description ?? 'Calculate',
			reason,
			stores: extra?.stores,
			import: createReasonDefinitionImporter({
				register: (id, def) => tool.register(id, def),
				persist: async (id, def) => {
					await tool.persist(id, def)
				},
			}),
		})
		return tool
	...
	it('imports a JSON definition file', async () => {
		...
		const tool = createToolWithImport()
	...
	it('persists imported definition to stores', async () => {
		...
		const tool = createToolWithImport({ stores })
	...
	it('imported definition survives across tool instances via stores', async () => {
		...
		const stores = storesFor({ path: storeDir.root })
		const tool1 = createToolWithImport({ stores })
...
describe('ReasonTool schema audit', () => {
	...
	it('groups items use additionalProperties', async () => {
		const defProps = topProps['definition']['properties'] as Record<string, Record<string, unknown>>
		const groupItems = defProps['groups']['items'] as Record<string, unknown>
		expect(groupItems['type']).toBe('object')
		expect(groupItems['additionalProperties']).toBe(true)
	})

	it('rules items use additionalProperties', async () => {
		const defProps = topProps['definition']['properties'] as Record<string, Record<string, unknown>>
		const ruleItems = defProps['rules']['items'] as Record<string, unknown>
		expect(ruleItems['type']).toBe('object')
		expect(ruleItems['additionalProperties']).toBe(true)
	})

	it('equations items use additionalProperties', async () => {
		const defProps = topProps['definition']['properties'] as Record<string, Record<string, unknown>>
		const eqItems = defProps['equations']['items'] as Record<string, unknown>
		expect(eqItems['type']).toBe('object')
		expect(eqItems['additionalProperties']).toBe(true)
	})

	it('bounds has minimum and maximum properties', async () => {
		const defProps = topProps['definition']['properties'] as Record<string, Record<string, unknown>>
		const boundsProps = defProps['bounds']['properties'] as Record<string, Record<string, unknown>>
		expect(boundsProps['minimum']['type']).toBe('number')
		expect(boundsProps['maximum']['type']).toBe('number')
	})

	it('definition requires type, id, and name', async () => {
		const def = topProps['definition'] as Record<string, unknown>
		expect(def['required']).toEqual(['type', 'id', 'name'])
	})
