...
import { createInterpret, InterpretTool } from '@scsr/core'
import { createInterpretDefinitionImporter, createMCPStoreManager } from '@scsr/server'
import { testTemplate, validateSchema } from '../../../../setup.js'
...
describe('InterpretTool', () => {
	...
	function makeTool(templates = [insuranceTemplate]) {
		...
		const tool = new InterpretTool({
			...
			interpreter,
			import: createInterpretDefinitionImporter({
				register: (template) => tool.register(template),
				persist: async (template) => {
					await tool.persist(template)
				},
			}),
		})
		return tool
